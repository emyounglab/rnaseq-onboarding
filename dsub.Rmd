...
annotations = read.csv("dsub/output/jgi_ergo_annotation_dsub.csv")
annotations = annotations[,-c(1,3)]

x <- read.delim("dsub/data/counts.txt") #5831 transcripts
x$GeneID = sub("|jgi.p\\|Debsub1\\|", "",x$GeneID)
...
```{r DGElist transformation, message=FALSE, warning=FALSE, include=F}
# transform data to DGEList
# grodown <- factor(c('cont', 'cont', 'cont', 'lowN', 'lowN', 'lowN', 'salt', 'salt', 'salt', 'lowFE', 'lowFE', 'lowFE')) #grodown 1 is control
names(x) = c('cont', 'cont', 'cont', 'lowN', 'lowN', 'lowN', 'salt', 'salt', 'salt', 'lowFE', 'lowFE', 'lowFE')
head(x)

cts <- as.matrix(x)
coldata = read.delim("dsub/data/LIBRARIES_data.txt")
coldata <- coldata[,c("X5.sampleName", "X6.conditionNumber")]

coldata = coldata[!duplicated(coldata),] #remove ddownlicates
coldata$X5.sampleName = sub("DHOP_", "", colnames(cts))
...
keep = rowSums(counts(dds)) >= 10 #filter out transcripts that the whole row adds down to less than 10
dds = dds[keep,]
...
```{r save nomalized counts, message=FALSE, warning=FALSE, include=F}
# save normalized counts
norm_counts = counts(dds, normalized = T)
write.table(norm_counts, file="dsub/output/norm_counts.txt", sep="\t", quote=F, col.names=T)
norm_counts = read.delim('dsub/output/norm_counts.txt')
```

```{r Transform normalized counts using rlog transformation, message=FALSE, warning=FALSE, include=F}
## improves distances/clustering for PCA and hierarchical clustering visualization
rld <- rlog(ddsColl, blind=TRUE)
```

```{r Plot PCA, echo=FALSE, fig.height=2, fig.width=5, message=FALSE, warning=FALSE}
# pca = plotPCA(rld, intgroup = 'condition')
# 
# png('dsub/output/images/dsub_pca.png')
# plot(pca)
# dev.off()
# plot(pca)

pdf("dsub/output/images/dsub_pca.pdf", height = 6, width = 12)
...
# ggsave("dsub/output/images/ds_pca3.png", plot = plotPCA2, height = 6, width = 12)
# 
...
# ggsave("dsub/output/images/ds_pca4.png", plot = plotPCA3, height = 6, width = 12)
```

```{r create function to save heatmap image, message=FALSE, warning=FALSE, include=FALSE}
...
save_pheatmap_png(hc, "dsub/output/images/dsub_hierCluster.png")
```
...
save(lowFE_contrastDEGenes, file = "dsub/output/lowFE_contrastDEGenes.RData")
...
save(lowN_contrastDEGenes, file = "dsub/output/lowN_contrastDEGenes.RData")
...
save(salt_contrastDEGenes, file = "dsub/output/salt_contrastDEGenes.RData")
...
write.csv(DEGS_annot, "dsub/output/annotatedDEG_ds.csv")

norm_counts <- norm_counts %>% rownames_to_column(var = "proteinId")

degs_annot_couts = merge(DEGS_annot, norm_counts,  by = "proteinId")
write.csv(degs_annot_couts, "dsub/output/annotatedDEGcounts_ds.csv")
...
write.csv(sigDEGS_annot, "dsub/output/annotatedsigDEG_ds.csv")

sigdegs_annot_couts = merge(sigDEGS_annot, norm_counts,  by = "proteinId")
write.csv(sigdegs_annot_couts, "dsub/output/annotatedsigDEGcounts_ds.csv")
```
...
write.csv(contrastDEGenes, file = "dsub/output/contrastDEGenes_ds.csv")
contrastDEGenes$proteinId = rownames(contrastDEGenes)
degs = contrastDEGenes[!duplicated(contrastDEGenes$proteinId),]
write.csv(degs, file = "dsub/output/unique_contrastDEGenes_ds.csv")
```
